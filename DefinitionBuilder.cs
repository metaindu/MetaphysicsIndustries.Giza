...
//            if (errors.Length > 0) throw new InvalidOperationException();
            ...
            foreach (Span sub in span.Subspans)
            {
                if (sub.Definition == grammar.def_1_definition)
                {
                    ...
                    {
                        if (sub2.Node == grammar.node_2_definition_identifier)
                        {
            ...
            foreach (Definition def in defs)
            {
                ...
                foreach (Span sub in defspan.Subspans)
                {
                    ...
                    else if (sub.Definition == null && sub.Node == grammar.node_3_definition__003D_) // '='
                    {
                        // skip it
                        continue;
                    }
                    ...
                    else if (sub.Definition == null && sub.Node == grammar.node_5_definition__003B_) // ';'
                    {
                        // skip it
                        continue;
                    }
        ...
        IEnumerable<DefmodItem> GetDefMods(Supergrammar grammar, Span span)
        {
            foreach (Span sub in span.Subspans)
            {
                if (sub.Node == grammar.node_2_defmod_defmod_002D_item ||
                    sub.Node == grammar.node_4_defmod_defmod_002D_item)
                {
        ...
        {
            if (span.Subspans[0].Node == grammar.node_1_defmod_item_id_002D_whitespace)
            {
                return DefmodItem.MindWhitespace;
            }
            if (span.Subspans[0].Node == grammar.node_2_defmod_item_id_002D_ignore)
            {
                if (span.Subspans[1].Node == grammar.node_4_defmod_item_id_002D_case ||
                    span.Subspans[2].Node == grammar.node_4_defmod_item_id_002D_case)
                {
                    return DefmodItem.IgnoreCase;
                }
            }
            if (span.Subspans[0].Node == grammar.node_5_defmod_item_id_002D_contiguous)
            {
...
//                nodes.Add(
        ...
        NodeBundle GetNodesFromSubExpr(Supergrammar grammar, Span span, Dictionary<string, Definition> defsByName)
        {
            ...
            foreach (Span sub in span.Subspans)
            {
                if (sub.Node == grammar.node_1_subexpr_identifier)
                {
                ...
                else if (sub.Node == grammar.node_5_subexpr__003A_) // ':'
                {
                    //skip it
                    continue;
                }
                else if (sub.Node == grammar.node_6_subexpr_tag)
                {
        ...
        List<Node> GetNodesFromLiteral(Supergrammar grammar, Span span)
        {
            ...
            for (i = 1; i < span.Subspans.Count - 1; i++)
            {
                ...
                if (sub.Node == grammar.node_1_literal__0027_) continue;
                if (sub.Node == grammar.node_6_literal__0027_) continue;
                ...
                else if (sub.Node == grammar.node_2_literal__005E__005C__005C__0027_) // [^\\']
                {
                    ch = sub.Value[0];
                }
                else if (sub.Node == grammar.node_3_literal__005C_) // '\\'
                {
        ...
        Node GetNodeFromCharClass(Supergrammar grammar, Span span)
        {
            ...
            for (i = 1; i < span.Subspans.Count - 1; i++)
            {
                ...
                if (sub.Node == grammar.node_2_charclass__005E__005C__005C__005C__005B__005C__005D_) // [^\\\[\]]
                {
                    items.Add(sub.Value[0]);
                }
                else if (sub.Node == grammar.node_3_charclass__005C_) // '\\'
                {
                    items.Add(sub.Value[0]);
                }
                else if (sub.Node == grammar.node_4_charclass_wldsrnt_005C__005C__005C__005B__005C__005D_) // [wldsrnt\\\[\]]
                {
                    items.Add(sub.Value[0]);
                }
                else if (sub.Node == grammar.node_5_charclass_unicodechar)
                {
        ...
        NodeBundle GetNodesFromOrExpr(Supergrammar grammar, Span span, Dictionary<string, Definition> defsByName)
        {
            ...
            foreach (Span sub in span.Subspans)
            {
                if (sub.Node == grammar.node_2_orexpr_expr ||
                    sub.Node == grammar.node_4_orexpr_expr)
                {
                    bundles.Add(GetNodesFromExpr(grammar, sub, defsByName));
                }
                else if (sub.Node == grammar.node_6_orexpr_modifier)
                {
